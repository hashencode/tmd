...
export interface TmCalendarProps {
  ...
  onChange?: (value?: string) => void; // 时间变化事件回调
  children?: any; // 子组件内容
...
function TmCalendar(props: TmCalendarProps) {
  const {
  ...
  useLayoutEffect(() => {
    ...
    if (!isDate(tmDefaultValue))
      throw new Error("Invalid value of tmDefaultValue");
    currentDate.current = {
  ...
  return (
    <View className={classNames("tm-calendar", className)} style={style}>
      {/* 头部 */}
      {!tmHideHead && (
